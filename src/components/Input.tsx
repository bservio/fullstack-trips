...
function Input({ className, error, errorMessage, ...props }: InputProps, ref: LegacyRef<HTMLInputElement> | undefined) {
	const inputClassName = twMerge(
		"rounded-lg border border-gray-300 bg-white p-2 text-sm font-normal text-secondary placeholder-black placeholder-opacity-20 outline-none trasition-all",
		error ? "border-red-500" : "focus:ring focus:ring-primary",
		className	
	);
